! Description
! Modules for the Graphic User's Interface
!
! Copyright:
...
SUBROUTINE RTTOV_GUI_LOAD( CHANNELS, FILE_COEF, FILE_SCAER, FILE_SCCLD, FILE_PCCOEF, NCHANNELS, ERR )
!
#include "throw.h"
...
TRY
  ...
  CALL RTTOV_GUI_HANDLELOAD(THE_RTH, CHANNELS, FILE_COEF, FILE_SCAER, FILE_SCCLD, FILE_PCCOEF, ERR )
  THROW(ERR.NE.0)
...
SUBROUTINE RTTOV_GUI_GET_EMISBRDF( PATH_ATLAS, FILENAME_PROF, FILENAME_SURFACE, MW_ATLAS_VER, &
   & IR_ATLAS_VER, VN_ATLAS_VER, ERR )
!
#include "throw.h"
...
!INTF_ON
  ...
  IMPLICIT NONE
  CHARACTER*(*),      INTENT(IN)  :: PATH_ATLAS, FILENAME_PROF, FILENAME_SURFACE
  INTEGER(KIND=JPIM), INTENT(IN)  :: MW_ATLAS_VER, IR_ATLAS_VER, VN_ATLAS_VER
  INTEGER(KIND=JPIM), INTENT(OUT) :: ERR
!INTF_END
...
! This interface is only valid for all channels, one profile
  ...
  IF( (CONTEXT%RTH%EMS_ATLAS_MONTH .NE. MONTH     .OR.  &
    &  CONTEXT%RTH%IR_ATLAS_VER .NE. IR_ATLAS_VER .OR.  &
    &  CONTEXT%RTH%MW_ATLAS_VER .NE. MW_ATLAS_VER ) .AND. &
    & DOTHERMAL ) THEN
    
    INFO("LOAD EMIS ATLAS")
    IF( CONTEXT%RTH%EMS_ATLAS_MONTH .GE. 1) THEN
      INFO("DROP PREVIOUS EMIS ATLAS")
      CALL RTTOV_DEALLOCATE_EMIS_ATLAS( CONTEXT%RTH%COEFS)
      CONTEXT%RTH%EMS_ATLAS_MONTH = -1_JPIM
      CONTEXT%RTH%IR_ATLAS_VER    = -1_JPIM
      CONTEXT%RTH%MW_ATLAS_VER    = -1_JPIM
    ENDIF
  ...
  IF( (CONTEXT%RTH%BRDF_ATLAS_MONTH .NE. MONTH     .OR. &
    ...
    CALL RTTOV_SETUP_BRDF_ATLAS( &
             ...
             &  VN_ATLAS_VER = VN_ATLAS_VER )
    THROW(ERR.NE.0)

    CONTEXT%RTH%BRDF_ATLAS_MONTH = MONTH
    CONTEXT%RTH%VN_ATLAS_VER     = VN_ATLAS_VER
...
!INTF_ON
  ...
  IMPLICIT NONE
    CHARACTER*(*), INTENT(IN)  :: FILENAME_PROF, FILENAME_SURFACE,  &
                                  FILENAME_RADR, FILENAME_KMAT,   &
                                  FILENAME_TRNS
...
!
  TYPE(RTTOV_CHANPROF), ALLOCATABLE :: CHANPROF(:)      ! Profile/channel indices (nchanprof)
  TYPE(TRANSMISSION_TYPE)           :: TRANSMISSION     ! transmittances and singlelayer optical depths (on User levels)
  TYPE(RADIANCE_TYPE    )           :: RADIANCEDATA     ! radiances (mw/cm-1/ster/sq.m) and degK
  LOGICAL(KIND=JPLM) ,     POINTER  :: CALCEMIS(:)    => NULL()  ! switches for emis calcs
  TYPE(RTTOV_EMISSIVITY),  POINTER  :: EMISSIVITY(:)  => NULL()  ! surface emis
  LOGICAL(KIND=JPLM) ,     POINTER  :: CALCREFL(:)    => NULL()  ! switches for refl calcs
  TYPE(RTTOV_REFLECTANCE), POINTER  :: REFLECTANCE(:) => NULL()  ! surface refl
  ...
  TYPE(RADIANCE_TYPE    )           :: RADIANCEDATA_K       
...
! Read profile and options from HDF
  ...
  IF(CONTEXT%OPTS%RT_IR%USER_AER_OPT_PARAM) THEN
    ...
    IF ( SIZE(AER_OPT_PARAM%ABS,1_JPIM) .NE. NCHANNELS ) THEN
      ERR = 1_JPIM
      THROWM(ERR.NE.0,"AEROSOL OPTICAL PARAMETERS DO NOT HAVE THE CORRECT NUMBER OF CHANNELS")
    ENDIF
    IF ( SIZE(AER_OPT_PARAM%ABS,2_JPIM) .NE. NLAYERS ) THEN
      ERR = 1_JPIM
  ...
  IF(CONTEXT%OPTS%RT_IR%USER_CLD_OPT_PARAM) THEN
    ...
    IF ( SIZE(CLD_OPT_PARAM%ABS,1_JPIM) .NE. NCHANNELS ) THEN
      ERR = 1_JPIM
      THROWM(ERR.NE.0,"CLOUD OPTICAL PARAMETERS DO NOT HAVE THE CORRECT NUMBER OF CHANNELS")
    ENDIF
    IF ( SIZE(CLD_OPT_PARAM%ABS,2_JPIM) .NE. NLAYERS ) THEN
      ERR = 1_JPIM
  ...
  ASW = 1 ! ALLOCATE
  CALL RTTOV_ALLOC_RAD( ERR, NCHANNELS, RADIANCEDATA, NLAYERS, ASW, INIT=.TRUE.)
  THROW(ERR.NE.0)
  ...
  CALL RTTOV_ALLOC_TRANSMISSION( ERR, TRANSMISSION, NLAYERS, NCHANNELS,  &
      & ASW, INIT = .TRUE._JPLM)  
  THROW(ERR.NE.0)
  ...
  IF( CONTEXT%RTTOV_RUN_MODE(1:1) .EQ. 'K' ) THEN
    ...
    CALL RTTOV_ALLOC_TRANSMISSION(             &
      ERR, TRANSMISSION_K, NLAYERS, NCHANNELS, &
      ASW, INIT = .TRUE. )
    THROW(ERR.NE.0)
    ...
    CALL RTTOV_ALLOC_RAD( ERR, NCHANNELS, RADIANCEDATA_K, NLAYERS, ASW, INIT=.TRUE.)
    THROW(ERR.NE.0)
...
! Deallocate
  ...
  ASW = 0_JPIM
  CALL RTTOV_ALLOC_RAD( ERR, NCHANNELS, RADIANCEDATA, NLAYERS, ASW)
  THROW(ERR.NE.0)
  ...
  IF( CONTEXT%RTTOV_RUN_MODE(1:1) .EQ. 'K' ) THEN
    ...
    CALL RTTOV_ALLOC_RAD( ERR, NCHANNELS, RADIANCEDATA_K, NLAYERS, ASW)
    THROW(ERR.NE.0)
...
!
  ...
  INTEGER(KIND=JPIM) :: NCHANNELS, NLEVELS, NLAYERS, NCHANPROF
  INTEGER(KIND=JPIM) :: ICHAN, I, J
  INTEGER(KIND=JPIM) :: ASW
  LOGICAL(KIND=JPLM) :: DOTHERMAL
  LOGICAL(KIND=JPLM) :: DOSOLAR
...
TRY
  ...
  SELECT CASE (TRIM(VARCH))
    ...
    CASE ("TT_CWN")
      R1 = THE_RTH%COEFS%COEF%TT_CWN
    CASE ("TT_A0")
      R1 = THE_RTH%COEFS%COEF%TT_A0
    CASE ("TT_A1")
      R1 = THE_RTH%COEFS%COEF%TT_A1
    CASE ("SS_CWN")
      R1 = THE_RTH%COEFS%COEF%SS_CWN
...
SUBROUTINE RTTOV_GUI_AER_CLIM_PROF( ML, P,T,Q,LEVSURF, &
...
!INTF_ON
  ...
  REAL(KIND=jprb),    INTENT(IN)  :: Q(ML)               ! Input water vapour profile (ppmv)
...
TRY
  ...
  IF( naer_max .NE. 13_JPIM ) THEN
    ERR = 1_JPIM
    THROWM(ERR.NE.0,"INCONSISTENCY  NAERMAX NOT EQUAL TO 13")
  ENDIF
