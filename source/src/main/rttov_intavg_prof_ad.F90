...
!
!--------------------------------------------------------------------
  USE rttov_types, ONLY : rttov_options, rttov_coef, rttov_coef_pccomp, profile_type
...
! --- Subroutine arguments
  ...
  TYPE(profile_type),      INTENT(INOUT) :: ProfGasIn_ad(SIZE(ProfIn))
...
! for rttov same set of weights for all elements, so one call suffices
   ...
   & ProfGasIn_ad(prof)%o3(istart(jo):iend(jo)) + zpz(istart(jo):iend(jo), jo) * ProfOut_ad(prof)%o3(jo)
      IF (opts%rt_mw%clw_data  ) ProfIn_ad(prof)%clw(istart(jo):iend(jo)) =      &
