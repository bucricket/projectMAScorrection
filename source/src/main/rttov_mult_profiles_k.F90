SUBROUTINE rttov_mult_profiles_k(profiles_k_rec, profiles_k, total_k_pc, opts)
! Copyright:
...
!INTF_ON
  ...
  REAL(KIND=jprb)    , INTENT(IN)    :: total_k_pc   (:, :, :)! dBT(PC)/dRadiance(RTTOV)
  TYPE(rttov_options), INTENT(IN)    :: opts
!INTF_END
  REAL(kind=jprb) :: prof_k_array(SIZE(total_k_pc, 1) * SIZE(total_k_pc, 3), &
                                  SIZE(profiles_k(1)%p), 5)
  REAL(kind=jprb) :: prof_k_array_surf(SIZE(total_k_pc, 1) * SIZE(total_k_pc, 3), 10)
  REAL(kind=jprb), ALLOCATABLE :: prof_k_array_aer(:,:,:), prof_k_array_cld(:,:,:)
  REAL(kind=jprb), ALLOCATABLE :: prof_k_array_cfrac(:,:), prof_k_array_icede(:,:)
...
! DAR: Re-pack predictor channel jacobians to improve data locality
  ...
  IF(opts%interpolation%LGRADP) THEN
    DO iprof = 1, nprofiles
      DO ichan = 1, nchannels_p
        ichan1 = ichan + (iprof - 1) * nchannels_p
        prof_k_array(ichan1, :, 5) = profiles_k(ichan1)%p
      ENDDO
    ENDDO
  ENDIF
...
! DAR: There is no variation in the training set in these gases for PC-RTTOV
