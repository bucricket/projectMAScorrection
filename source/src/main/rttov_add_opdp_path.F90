!
SUBROUTINE rttov_add_opdp_path(opts, opdp_path, opdp_pathA, opdp_pathB)
! Description:
...
! scalar arguments with intent(out):
  ...
  TYPE(opdp_path_Type), INTENT(IN)    :: opdp_pathA
  TYPE(opdp_path_Type), INTENT(IN)    :: opdp_pathB
!INTF_END
!- End of header --------------------------------------------------------
  opdp_path%atm_level = opdp_pathA%atm_level + opdp_pathB%atm_level
  IF (opts%rt_ir%addsolar) THEN
    opdp_path%sun_level_path2 = opdp_pathA%sun_level_path2 + opdp_pathB%sun_level_path2
  ENDIF
END SUBROUTINE 
