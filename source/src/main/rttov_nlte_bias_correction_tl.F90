SUBROUTINE rttov_nlte_bias_correction_tl(coef, profiles, profiles_tl, &
                                         angles, chanprof, &
                                         rad_tl) ! INOUT
! Copyright:
...
!INTF_END
  ...
  REAL(jprb)    :: t_tl(coef%nlte_coef%ncoef,SIZE(profiles))
...
!===============================================================================!
  DO iprof = 1, SIZE(profiles)
    ...
    IF(profiles(iprof)%sunzenangle > nlte_coef%sol_zen_angle(nlte_coef%nsol) &
      .OR. profiles(iprof)%sunzenangle < 0._jprb) THEN ! below horizon do nothing
      do_nlte(iprof) = .FALSE.
      CYCLE
    ENDIF
    ...
    IF(do_nlte(prof)) THEN
      ...
      IF(chan >= nlte_coef%start_chan .AND. chan <= nlte_coef%end_chan) THEN  
        ...
        rad_tl%clear(i)  = rad_tl%clear(i) + &
                           DOT_PRODUCT(d_rad(:), t_tl(:,prof))
        rad_tl%cloudy(i) = rad_tl%cloudy(i) + &
                           DOT_PRODUCT(d_rad(:), t_tl(:,prof))
        rad_tl%total(i)  = rad_tl%total(i) + &
                           DOT_PRODUCT(d_rad(:), t_tl(:,prof))
      ENDIF
    ENDIF
  ENDDO
