SUBROUTINE rttov_nlte_bias_correction_k(coef,     &
                                         profiles,   profiles_k, &
                                         angles,   &
                                         chanprof, &
                                                     rad_k) 
...
!INTF_END
  ...
  REAL(jprb)    :: t_k(coef%nlte_coef%ncoef,SIZE(chanprof))
...
!Note that macros contain prof (from chanprof) not iprof (counter) as in D/TL/AD 
#define lb1 bounds(1,1,prof)
#define ub1 bounds(2,1,prof)
#define lb2 bounds(1,2,prof)
#define ub2 bounds(2,2,prof)
  ...
  DO i = 1, SIZE(chanprof)
    ...
    IF(do_nlte(prof)) THEN
      ...
      IF(chan >= nlte_coef%start_chan .AND. chan <= nlte_coef%end_chan) THEN  
        ...
        ! only take into account contribution from rad_k%total because 
        ! perturbation from rad_k%clear = 0 by definition

        t_k(1, i) = t_k(1, i) + rad_k%total(i) * d_rad(1)
        t_k(2, i) = t_k(2, i) + rad_k%total(i) * d_rad(2)
        ! t_k(3,:) later zero'd
!       t_k(3, i) = t_k(3, i) + rad_k%total(i) * d_rad(3)
      ENDIF
    ENDIF
  ENDDO
