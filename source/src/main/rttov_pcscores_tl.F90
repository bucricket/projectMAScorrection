...
!INTF_END
  ...
  INTEGER(KIND=jpim) :: nchannels, nchannels_p             ! Number of radiances computed (channels used * profiles)
...
! nchannels now no. simulated channels per prof
  ...
  DO prof = 1, nprofiles
    DO j = (prof-1) * npcscores_p + 1, prof * npcscores_p
      ...
      pccomp_tl%pcscores(j) = DOT_PRODUCT( &
        ...
        radiancedata_tl%total((prof-1)*nchannels_p + 1 : prof*nchannels_p) * &
