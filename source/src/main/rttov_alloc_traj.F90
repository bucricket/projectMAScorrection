...
!! @param[in]     nchannels      total number of channels being simulated
...
!INTF_ON
  ...
  USE rttov_types, ONLY :  &
       ...
       & rttov_traj
!INTF_OFF
  USE rttov_const, ONLY : ncldtyp, sensor_id_hi, sensor_id_ir
...
#include "rttov_alloc_trans_scatt_ir.interface"
    ...
    IF (asw .EQ. 1_jpim) THEN
      ...
      ALLOCATE (traj%profiles_dry(nprofiles), STAT = err)
      THROWM(err .NE. 0, "allocation of traj%profiles_dry")
! ON LEVELS
! ---------
      ...
      ! Only need to allocate gases for profiles_dry (so p and t are redundant)
      CALL rttov_alloc_prof( &
            & err,                &
            & nprofiles,          &
            & traj%profiles_dry,  &
...
!
      ...
      CALL rttov_alloc_ircld( &
            ...
            & direct = direct)
      THROW(err .NE. 0)
      ALLOCATE (traj%thermrefl(nchannels), STAT = err)
      THROWM(err.NE.0,"Allocation of thermrefl failed")
      ...
      ENDIF
      IF ((coefs%coef%id_sensor == sensor_id_hi .OR. coefs%coef%id_sensor == sensor_id_ir) .AND. &
          coefs%coef%fmv_model_ver == 9 .AND. opts%rt_ir%addsolar) THEN
    ...
    ELSE
      ...
      CALL rttov_alloc_aux_prof( &
            ...
            & alloc_layer_vars = .FALSE._jplm)
      THROW(err.NE.0)
      DEALLOCATE (traj%thermrefl, STAT = err)
      THROWM(err.NE.0,"DeAllocation of thermrefl failed")
! ON LAYERS
! ---------
!
      CALL rttov_alloc_ircld( &
...
!
      ...
      CALL rttov_alloc_prof( &
            ...
            & 0_jpim)
      THROW(err .NE. 0)
      DEALLOCATE (traj%profiles_dry, STAT = err)
      THROWM(err .NE. 0, "deallocation of traj%profiles_dry")
