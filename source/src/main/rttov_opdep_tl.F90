...
!  1.10   10/01/2013  Add PMC shifts (P Rayer)
!
      ...
      DO ii = 1, coef%nmixed
        DO lay = 1, nlayers
          opticaldepth_tl(lay, j) =      &
            & opticaldepth_tl(lay, j) + fast_coef%mixedgas(lay, chan, ii) * predictors_tl%mixedgas(ii, lay, prof)
        ENDDO
      ENDDO
    ENDIF
  ENDDO
  ...
  DO j = 1, nchannels
    ...
    IF (coef%nwater == 15) THEN
      DO lay = 1, nlayers
        ...
        t2 = fast_coef%watervapour(lay, chan, 5) * predictors_tl%watervapour(5, lay, prof) +      &
          ...
          & fast_coef%watervapour(lay, chan, 8) * predictors_tl%watervapour(8, lay, prof)
        opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + t1 + t2
      ENDDO
      DO lay = 1, nlayers
        t1 = fast_coef%watervapour(lay, chan, 9) * predictors_tl%watervapour(9, lay, prof) +      &
        ...
        t2 = fast_coef%watervapour(lay, chan, 13) * predictors_tl%watervapour(13, lay, prof) +      &
          & fast_coef%watervapour(lay, chan, 14) * predictors_tl%watervapour(14, lay, prof) +       &
          & fast_coef%watervapour(lay, chan, 15) * predictors_tl%watervapour(15, lay, prof)
        opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + t1 + t2
      ENDDO
    ELSE
      DO ii = 1, coef%nwater
        DO lay = 1, nlayers
          opticaldepth_tl(lay, j) =      &
            & opticaldepth_tl(lay, j) + fast_coef%watervapour(lay, chan, ii) * predictors_tl%watervapour(ii, lay, prof)
        ENDDO
      ENDDO
    ENDIF
  ENDDO
    ...
    DO j = 1, nchannels
      ...
      ELSE
        DO ii = 1, coef%nozone
          DO lay = 1, nlayers
            opticaldepth_tl(lay, j) =      &
              & opticaldepth_tl(lay, j) + fast_coef%ozone(lay, chan, ii) * predictors_tl%ozone(ii, lay, prof)
          ENDDO
        ENDDO
      ENDIF
    ENDDO
...
!-----------
  IF (coef%nco2 > 0) THEN
    DO j = 1, nchannels
      ...
      DO ii = 1, coef%nco2
        DO lay = 1, nlayers
          opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + fast_coef%co2(lay, chan, ii) * predictors_tl%co2(ii, lay, prof)
        ENDDO
      ENDDO
    ENDDO
  ENDIF
...
!       DO ii = 1, coef%nn2o
!         DO lay = 1, nlayers
!           opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + fast_coef%n2o(lay, chan, ii) * predictors_tl%n2o(ii, lay, prof)
!         ENDDO
!       ENDDO
!     ENDDO
!   ENDIF
...
!       DO ii = 1, coef%nco
!         DO lay = 1, nlayers
!           opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + fast_coef%co(lay, chan, ii) * predictors_tl%co(ii, lay, prof)
!         ENDDO
!       ENDDO
!     ENDDO
!   ENDIF
...
!       DO ii = 1, coef%nch4
!         DO lay = 1, nlayers
!           opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + fast_coef%ch4(lay, chan, ii) * predictors_tl%ch4(ii, lay, prof)
!         ENDDO
!       ENDDO
!     ENDDO
!   ENDIF
    ...
    DO j = 1, nchannels
      ...
      DO ii = 1, coef%pmc_nvar
        DO lay = 1, coef%pmc_nlay
          opticaldepth_tl(lay, j) = opticaldepth_tl(lay, j) + coef%pmc_coef(lay, chan, ii) * predictors_tl%pmc(ii, lay, prof, chan)
        ENDDO
      ENDDO
    ENDDO
  ENDIF
