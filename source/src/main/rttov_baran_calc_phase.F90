...
REAL(KIND=JPRB),INTENT(IN)  :: G          ! Asymetry factor
REAL(KIND=JPRB),INTENT(IN)  :: PHANGLE(:) ! angles(degrees)
REAL(KIND=JPRB),INTENT(OUT) :: PHASE(:)   ! phase function
!INTF_END
...
NPHANGLE = SIZE(PHANGLE)
  ...
  IF ( G .GE. 0.7_JPRB ) THEN
    IF ( ( G .LE. 0.8_JPRB ) .AND. ( G .GE. 0.7_JPRB ) ) THEN
      ...
      ALPHA=NORM/SQRT(G)
    ENDIF
    IF ( ( G .LE. 0.9_JPRB ) .AND. ( G .GE. 0.8_JPRB ) ) THEN
