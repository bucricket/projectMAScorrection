PROGRAM example_cld_file_fwd
  ...
  !     *************************************************************
  !
  !     TEST PROGRAM FOR RTTOV FORWARD MODEL WITH CLOUDS
  !          RTTOV VERSION 11
  !
  ! Demonstrating how to run RTTOV IR cloud simulations using the
  ...
  !
  ! The script run_example_cld_file_fwd.sh may be used to run this program.
  ! The output is generated in a file called example_cld_file_fwd_output.dat.
  !
  ! NB This program is very similar to example_fwd.F90 with the addition
  !    of the cloud scattering.
  !
  ! NB Although this example is set up to allow solar simulations, cloud
  !    coefficient files are not currently provided which include visible/NIR
  !    channels.
  !
...
#include "rttov_skipcommentline.interface"
  ...
  READ(*,*) coef_filename
  WRITE(0,*) 'enter name of cloud coefficient file (in current directory)'
  READ(*,*) cld_coef_filename
  WRITE(0,*) 'enter name of file containing (clear-sky) profile data (in current directory)'
  READ(*,*) prof_filename
  WRITE(0,*) 'enter name of file containing cloud profile data (in current directory)'
...
!     CALL rttov_skipcommentline(iup, errorstatus)
    ...
    ! (Ignored for full cloud simulations)
  ...
  ! The cloud coefficient files contain optical parameters for 5 pre-defined water cloud
  ! types and for the hexagonal and aggregate ice cloud parameterisations.

  ! RTTOV also provides parameterisations of the Baran ice optical property database. It is
  ! recommended to use the Baran scheme in preference to the older hexagonal or aggregate
  ! particle types.

  ! Cloud simulations are performed by specifying liquid water and ice water content profiles
  ! for one or more of these particle types in the profile%cloud(:,:) array and a profile
  ! of cloud fractions in profile%cfrac(:)
  ...
  DO iprof = 1, nprof
    ...
    ! Read cloud liquid and ice water content and cloud fraction profiles
    DO lay = 1, nlevels-1
