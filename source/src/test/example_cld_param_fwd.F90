PROGRAM example_cld_param_fwd
  ...
  !          RTTOV VERSION 11
  !
  ! Demonstrating how to supply cloud optical parameters
  ! explicitly to RTTOV.
  !
  ...
  !
  ! The script run_example_cld_param_fwd.sh may be used to run this program.
  ! The output is generated in a file called example_cld_param_fwd_output.dat.
  !
...
#include "rttov_skipcommentline.interface"
  ...
  READ(*,*) prof_filename
  WRITE(0,*) 'enter name of cloud optical parameter file (in current directory)'
...
!     CALL rttov_skipcommentline(iup, errorstatus)
  ...
  DO iprof = 1, nprof
    ...
    DO i = 1, nlevels-1_jpim
      READ(iup,*) cld_opt_param % ABS(joff:joff+nchannels-1,i)
    ENDDO
    CALL rttov_skipcommentline(iup, errorstatus)

    DO i = 1, nlevels-1_jpim
      READ(iup,*) cld_opt_param % sca(joff:joff+nchannels-1,i)
    ENDDO
    CALL rttov_skipcommentline(iup, errorstatus)
    ...
    DO i = 1, nlevels-1_jpim
      DO j = 1, nchannels
        READ(iup,*) cld_opt_param % pha(joff+j-1,i,:)
      ENDDO
    ENDDO
    CALL rttov_skipcommentline(iup, errorstatus)
  ENDDO
  ...
  CALL rttov_bpr_init(cld_opt_param % phangle(:), errorstatus)
  IF (errorstatus /= errorstatus_success) THEN
  ...
  DO iprof = 1, nprof
    DO i = 1, nlevels-1_jpim
      DO j = 1, nchannels
        ...
        IF (ANY(cld_opt_param % pha(j+joff,i,:) > 0._jprb)) THEN

          ! If this phase function is same as previous layer copy the bpr
          IF (i > 1 .AND. ALL(cld_opt_param % pha(j+joff,i,:) == cld_opt_param % pha(j+joff,MAX(i-1,1),:))) THEN
            cld_opt_param % bpr(j+joff,i) = cld_opt_param % bpr(j+joff,i-1)
          ELSE
            CALL rttov_bpr_calc(cld_opt_param % pha(j+joff,i,:), &! input phase function
                              & cld_opt_param % phangle(:),      &! input phase angles
                              & cld_opt_param % bpr(j+joff,i),   &! output b parameter
                              & errorstatus)
            IF (errorstatus /= errorstatus_success) THEN
        ...
        ELSE
          cld_opt_param % bpr(j+joff,i) = 0._jprb
        ENDIF
