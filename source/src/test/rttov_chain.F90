MODULE RTTOV_CHAIN
  ...
  !
  ! Description:
  ! Builds and handles chained list of data 
  ...
  !          
  ! History:
  ! 14/08/2008 Add profile members %be and %cosbk


USE PARKIND1, ONLY: JPIM, JPRB, JPLM
USE RTTOV_TYPES, ONLY: PROFILE_TYPE, S2M_TYPE, SSKIN_TYPE
...
TYPE(CHAIN), POINTER :: C
INTEGER(KIND=JPIM) :: LBOUNDA(7), UBOUNDA(7)
...
RECURSIVE SUBROUTINE PRINT_ARRAY( LUN, FORMAT, A0, A1, A2, A3, A4, A5, A6, A7 )
...
SUBROUTINE chain_profile_type( err, chn, name, a0, a1, a2, a3, a4, a5, a6, a7, p0, p1, p2, p3, p4, p5, p6, p7 )
  INTEGER(KIND=JPIM),                    INTENT(OUT)   :: err
  TYPE(chain),                           INTENT(INOUT) :: chn
  CHARACTER,                             INTENT(IN)    :: name*( * )
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a0
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a1(:)
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a2(:,:)
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a3(:,:,:)
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a4(:,:,:,:)
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a5(:,:,:,:,:)
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a6(:,:,:,:,:,:)
  TYPE(profile_type), OPTIONAL, TARGET,  INTENT(IN)    :: a7(:,:,:,:,:,:,:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p0
  TYPE(profile_type), OPTIONAL, POINTER                :: p1(:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p2(:,:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p3(:,:,:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p4(:,:,:,:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p5(:,:,:,:,:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p6(:,:,:,:,:,:)
  TYPE(profile_type), OPTIONAL, POINTER                :: p7(:,:,:,:,:,:,:)
          ...
          DO i5 = ubounda(5), lbounda(5), -1
            ...
            DO i6 = ubounda(6), lbounda(6), -1
              ...
              DO i7 = ubounda(7), lbounda(7), -1
                ...
                ENDIF
                CALL chain_real_JPRB( err, chn, trim( str ) // "%COSBK", a0 = p%cosbk )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%BE", a0 = p%be )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%CFRACTION", a0 = p%cfraction )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%CTP", a0 = p%ctp )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%LATITUDE", a0 = p%latitude )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%ELEVATION", a0 = p%elevation )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%SUNAZANGLE", a0 = p%sunazangle )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%SUNZENANGLE", a0 = p%sunzenangle )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%AZANGLE", a0 = p%azangle )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%ZENANGLE", a0 = p%zenangle )
                IF( err .NE. 0 ) RETURN
                CALL chain_s2m_type( err, chn, trim( str ) // "%S2M", a0 = p%s2m )
                IF( err .NE. 0 ) RETURN
                CALL chain_sskin_type( err, chn, trim( str ) // "%SKIN", a0 = p%skin )
                IF( err .NE. 0 ) RETURN
...
END SUBROUTINE chain_profile_type
...
SUBROUTINE chain_sskin_type( err, chn, name, a0, a1, a2, a3, a4, a5, a6, a7, p0, p1, p2, p3, p4, p5, p6, p7 )
  INTEGER(KIND=JPIM),                  INTENT(OUT)   :: err
  TYPE(chain),                         INTENT(INOUT) :: chn
  CHARACTER,                           INTENT(IN)    :: name*( * )
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a0
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a1(:)
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a2(:,:)
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a3(:,:,:)
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a4(:,:,:,:)
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a5(:,:,:,:,:)
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a6(:,:,:,:,:,:)
  TYPE(sskin_type), OPTIONAL, TARGET,  INTENT(IN)    :: a7(:,:,:,:,:,:,:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p0
  TYPE(sskin_type), OPTIONAL, POINTER                :: p1(:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p2(:,:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p3(:,:,:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p4(:,:,:,:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p5(:,:,:,:,:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p6(:,:,:,:,:,:)
  TYPE(sskin_type), OPTIONAL, POINTER                :: p7(:,:,:,:,:,:,:)
  INTEGER(KIND=JPIM)                                 :: nargs
  INTEGER(KIND=JPIM)                                 :: lbounda(7)
  INTEGER(KIND=JPIM)                                 :: ubounda(7)
  INTEGER(KIND=JPIM)                                 :: i1
  ...
  999 CONTINUE
  err = errorstatus_fatal
END SUBROUTINE chain_sskin_type
SUBROUTINE chain_s2m_type( err, chn, name, a0, a1, a2, a3, a4, a5, a6, a7, p0, p1, p2, p3, p4, p5, p6, p7 )
  INTEGER(KIND=JPIM),                INTENT(OUT)   :: err
  TYPE(chain),                       INTENT(INOUT) :: chn
  CHARACTER,                         INTENT(IN)    :: name*( * )
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a0
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a1(:)
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a2(:,:)
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a3(:,:,:)
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a4(:,:,:,:)
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a5(:,:,:,:,:)
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a6(:,:,:,:,:,:)
  TYPE(s2m_type), OPTIONAL, TARGET,  INTENT(IN)    :: a7(:,:,:,:,:,:,:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p0
  TYPE(s2m_type), OPTIONAL, POINTER                :: p1(:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p2(:,:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p3(:,:,:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p4(:,:,:,:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p5(:,:,:,:,:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p6(:,:,:,:,:,:)
  TYPE(s2m_type), OPTIONAL, POINTER                :: p7(:,:,:,:,:,:,:)
  INTEGER(KIND=JPIM)                               :: nargs
  INTEGER(KIND=JPIM)                               :: lbounda(7)
  INTEGER(KIND=JPIM)                               :: ubounda(7)
  INTEGER(KIND=JPIM)                               :: i1
