...
type(transmission_type),intent(inout)::x
...
type(transmission_type),intent(out)::x
...
subroutine rttov_hdf_transmission_wh(x,lun,err,compress)
type(transmission_type),intent(in)::x
integer(hid_t),intent(in)::lun
integer(kind=jpim),intent(out)::err
...
if(associated(x%TAU_LEVELS))then
  call write_array_hdf(lun,sname,&
    ...
    & err,r2=x%TAU_LEVELS  ,compress=compress)
  THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
endif
...
if(associated(x%TAUSUN_LEVELS_PATH2))then
  call write_array_hdf(lun,sname,&
    ...
    & err,r2=x%TAUSUN_LEVELS_PATH2  ,compress=compress)
  THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
endif
...
if(associated(x%TAUSUN_LEVELS_PATH1))then
  call write_array_hdf(lun,sname,&
    ...
    & err,r2=x%TAUSUN_LEVELS_PATH1  ,compress=compress)
  THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
endif
