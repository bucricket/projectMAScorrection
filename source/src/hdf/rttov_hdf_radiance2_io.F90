...
type(radiance2_type),intent(inout)::x
...
type(radiance2_type),intent(out)::x
...
subroutine rttov_hdf_radiance2_wh(x,lun,err,compress)
type(radiance2_type),intent(in)::x
integer(hid_t),intent(in)::lun
integer(kind=jpim),intent(out)::err
...
if(associated(x%UP))then
  call write_array_hdf(lun,sname,&
    ...
    & err,r2=x%UP , units = 'mw/cm-1/ster/sq.m' ,compress=compress)
  THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
endif
...
if(associated(x%DOWN))then
  call write_array_hdf(lun,sname,&
    ...
    & err,r2=x%DOWN , units = 'mw/cm-1/ster/sq.m' ,compress=compress)
  THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
endif
...
if(associated(x%SURF))then
  call write_array_hdf(lun,sname,&
    ...
    & err,r2=x%SURF , units = 'mw/cm-1/ster/sq.m' ,compress=compress)
  THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
endif
