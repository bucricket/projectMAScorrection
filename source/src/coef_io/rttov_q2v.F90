SUBROUTINE rttov_q2v (&
  ...
  !
  ! Description:
  !   Conversion of specific concentration (wet air) to volume mixing ratio (wet air).
...
!INTF_ON
  ...
  REAL(KIND=jprb),    INTENT(IN)    :: q_gas    ! specific concentration for gas (kg/kg)
  REAL(KIND=jprb),    INTENT(INOUT) :: v_gas    ! volume mixing ratio for gas (ppmv)
!INTF_END
