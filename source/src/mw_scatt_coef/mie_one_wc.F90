subroutine mie_one_wc ( wavelength, f_ghz, i_type, ll_melt, dia_froz, &
...
!INTF_OFF
use mod_mie,  only: i_snow, i_graupel, i_aggregate, get_d_dia
!INTF_ON
implicit none
...
! Local variables
real    (kind=jprb) :: itgr, perm_re, perm_im
...
if (ll_melt .and. (i_type == i_snow .or. i_type == i_graupel .or. i_type == i_aggregate) ) then
  ...
  call perm_water (f_ghz, 273.0_jprb, perm_re, perm_im)
  perm_wat  = cmplx (perm_re, perm_im, jprb)

  call perm_ice   (f_ghz, 273.0_jprb, perm_re, perm_im)
