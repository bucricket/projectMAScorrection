...
def readChannelData(testDir, fileName, varName, profList, chanList, nlevels=1, model='direct'):
    ...
    result = readSetUp(testDir, fullFileName, profList, chanList)
    if result:
        chanprof, ichanprof = result
    else:
        return np.array([])

    # Read entire data file
    with file(fullFileName, 'r') as f:
        lines = f.readlines()
...
def readJacobians(testDir, varName, profList, chanList, pcrec=None, icldaer=0):
    ...
    # Construct output full path to output file
    if pcrec:
        fullFileName = testDir + '/out/k/profiles_k_rec.txt'
    else:
        fullFileName = testDir + '/out/k/profiles_k.txt'
...
def readNameListVar(testDir, profList, fileName, varName):
