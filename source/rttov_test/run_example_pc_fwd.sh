...
COEF_FILENAME="rtcoef_metop_2_iasi.H5"    # Location of this file is set below in $COEF_DIR
PCCOEF_FILENAME="pccoef_metop_2_iasi.H5"  # Location of this file is set below in $PCCOEF_DIR
...
TEST_DIR=./test_example_pc_fwd.1

# Paths relative to the rttov_test/${TEST_DIR} directory:
BIN_DIR=../../$BIN                           # BIN directory (may be set with BIN= argument)
REF_TEST_DIR=../test_example_pc_fwd.2        # Test reference data
COEF_DIR=../../rtcoef_rttov11/rttov9pred101L # Coefficients directory
PCCOEF_DIR=../../rtcoef_rttov11/pc           # PC coefficients directory
...
rm -f $PCCOEF_FILENAME
if [ -s $PCCOEF_DIR/$PCCOEF_FILENAME ]; then
  ln -s $PCCOEF_DIR/$PCCOEF_FILENAME
else
  echo "PC coef file $PCCOEF_DIR/$PCCOEF_FILENAME not found, aborting..."
  exit 1
fi
...
rm -f $COEF_FILENAME $PCCOEF_FILENAME
