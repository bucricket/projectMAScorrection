...
from rttov_test_plot_mod import readChannelData, readJacobians, \
...
PROF_VARS = ('T', 'Q', 'log(Q)', 'O3', 'CO2', 'CO', 'N2O', 'CH4', 'skin%T', 'aerosols', 'cfrac', 'cloud', 'icede', 'P')
...
class rttov_test_plot():
    def __init__(self, ntests=3):
        ...
        mainfgs = wx.FlexGridSizer(rows=1, cols=2, hgap=5)
        self.mainFrame.SetSizer(mainfgs)
        # -----------------------------------------------------

        # --- Sizer for LHS of GUI - tests --------------------
        leftfgs = wx.FlexGridSizer(rows=self.ntests+1, cols=1, vgap=5)
        ...
        rightfgs = wx.FlexGridSizer(rows=3, cols=1, vgap=5)
        ...
        self.combo_plottype.SetSelection(0)
        ...
        self.combo_cldaertype.AppendItems(map(str, range(1, CLDAER_MAXINDEX+1)))
        ...
        plotfgs.Add(label, flag = wx.ALIGN_LEFT | wx.ALIGN_CENTER_VERTICAL)
        self.slider_legend_alpha = wx.Slider(plotpanel, wx.ID_ANY, size=(textwidth,-1))
        ...
        plotfgs.Add(label, flag = wx.ALIGN_LEFT | wx.ALIGN_CENTER_VERTICAL)
        self.text_legend_fontsize = wx.TextCtrl(plotpanel, wx.ID_ANY, '', size=(textwidth,-1))
    ...
    def readData(self):
        """ Read data for each test dir into self.testData[:] list. Any associated pressure
            profile data is in self.pData[:]. Any necessary calculations are performed so
            that the contents of self.testData[:] are precisely the data to be plotted/written.
        ...
        for t in range(self.ntests):
            if self.btn_addtoplot[t].GetValue():
                ...
                if self.text_proflist[t].GetValue():
                    try:
                        self.profLists.append(map(int, self.text_proflist[t].GetValue().split(',')))
                    except:
                ...
                if self.text_chanlist[t].GetValue():
                    try:
                        self.chanLists.append(map(int, self.text_chanlist[t].GetValue().split(',')))
                    except:
        ...
        for i, t in enumerate(self.testDirs):
            ...
            if PLOT_OPTS[self.iPlotData] in ['Radiances', 'BTs', 'Reflectances']:
                ...
                if 'thermal' in tauOpt:
                    ...
                    varName = 'TAUSUN_LAYERS_PATH1'
                else:
                    varName = 'TAUSUN_LAYERS_PATH2'
            ...
            elif PLOT_OPTS[self.iPlotData] in ['Jacobian', 'Jacobian - PC rec.']:
                ...
                if varName.upper() == 'LOG(Q)':
                    ...
                    data = readJacobians(t, 'Q', self.profLists[i], self.chanLists[i], icldaer=icldaer, pcrec=pcrec)
                    q = readProfileVar(t, self.profLists[i], 'q.txt')
                    if self.diffTestDir:
                        diffData = readJacobians(self.diffTestDir, 'Q', self.profLists[i], self.chanLists[i], icldaer=icldaer, pcrec=pcrec)
                    ...
                    data = readJacobians(t, varName.upper(), self.profLists[i], self.chanLists[i], icldaer=icldaer, pcrec=pcrec)
                    if self.diffTestDir:
                        diffData = readJacobians(self.diffTestDir, varName.upper(), self.profLists[i], self.chanLists[i], icldaer=icldaer, pcrec=pcrec)
                    if not self.checkData(data, diffData):
                        return 1
                ...
                if varName.upper() == 'LOG(Q)':
                    ...
                    data = np.log(data)
                    if self.diffTestDir:
                        diffData = np.log(diffData[ip,ic,:])
    ...
    def plotData(self):
        """ Generate plot from contents of self.testData[:] and (if required)
            self.pData[:]. The plot type is determined by self.plotType[:] """
        ...
        if self.plotType[0] == 'barchan':
            ...
            nprof = sum(map(len, self.profLists))
            ind = range(nchan)
            ...
            for it, data in enumerate(self.testData):
                for ip in range(len(self.profLists[it])):
                    ...
                    if barplot:
                        plt.bar(map(lambda x:x + i*width, ind), data[ip][:], width, label=label, color=colours[i % len(colours)])
                    else:
            ...
            if barplot:
                ...
                plt.xticks(map(lambda x:x + 0.5 * width * nprof, ind), self.chanNameLists[0])
        ...
        if self.text_plot_title.GetValue():
            plt.title(string.replace(self.text_plot_title.GetValue(), '\\n', '\n'))
        #else:
            #plt.title(title)
    ...
    def writeData(self, filename, verbose=False):
        ...
        with file(filename, 'w') as f:
            for it, data in enumerate(self.testData):
                ...
                if self.plotType[0] == 'barchan':
                    if verbose:
                        f.write('# Chan list: ' + ','.join(map(str, self.chanLists[0])) + '\n')
                    for ip in range(len(self.profLists[it])):
                        if verbose:
                            f.write('# Prof: ' + str(self.profLists[it][ip]) + '\n')
                        f.write(','.join(map(str, data[ip])) + '\n')
                    ...
                    for ip in range(len(self.profLists[it])):
                        ...
                        for ic in range(len(self.chanLists[it])):
                            ...
                            f.write(','.join(map(str, data[ip][ic])) + '\n')
        return 0
    # -------------------------------------------------------
    ...
    def calcStatData(self, data):
        ...
        selStats = map(lambda x: int(x.GetValue()), self.btn_stats)
        if sum(selStats) == 0:
            # No stats selected
            return (None, ) * 3
