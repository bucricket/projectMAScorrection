...
from prof_gen import make_dirs, write_list, \
                     ppmvdry2kgkgwet, ppmvdry2ppmvwet, kgkgwet2ppmvdry, kgkgwet2ppmvwet, \
...
def read_div52(fname_prof, fname_surf, fname_co2='', nprof=52, nlev=101, gas_unit=UNIT_PPMVWET):
    """Reads the diverse 52 profile set as provided on the NWP SAF web site.
       ...
       Units for gases are kg/kg dry so they must be converted to ppmv wet.
        ...
        for p in range(nprof):
            ...
            for l in range(nlev):
                ...
                for i, k in enumerate(keys):
                    if k == 'q':
                        qkgkgwet = data[i]
                        profsq[p].append(kgkgwet2ppmvdry(data[i], 'q')) # Store q in ppmv dry for CO2 unit conversion
                    # Conversion from kg/kg wet to selected units
                    if (k not in GASMASSLIST) or gas_unit == UNIT_KGKGWET:
                        profs[p][k].append(data[i])
                    elif gas_unit == UNIT_PPMVWET:
                        profs[p][k].append(kgkgwet2ppmvwet(data[i], k, qkgkgwet))
                    else: # gas_unit == UNIT_PPMVDRY:
                        profs[p][k].append(kgkgwet2ppmvdry(data[i], k))
    ...
    with file(fname_surf, 'r') as f:
        ...
        for p in range(nprof):
            data = map(float, f.readline().split())
            for i, k in enumerate(keys):
                if k[:1] == 'q': qkgkgwet = data[i]
                # Conversion from kg/kg wet to selected units
                if (k[:-2] not in GASMASSLIST) or gas_unit == UNIT_KGKGWET:
                    profs[p][k] = data[i]
                elif gas_unit == UNIT_PPMVWET:
                    profs[p][k] = kgkgwet2ppmvwet(data[i], k[:-2], qkgkgwet)
                else: # gas_unit == UNIT_PPMVDRY:
                    profs[p][k] = kgkgwet2ppmvdry(data[i], k[:-2])
...
def read_div83(fname, nprof=83, nlev=101, gas_unit=UNIT_PPMVWET):
            ...
            for l in range(nlev):
                ...
                for i, k in enumerate(keys):
                    ...
                    if (k not in GASMASSLIST) or gas_unit == UNIT_PPMVDRY:
                        ...
                        profs[p][k].append(ppmvdry2kgkgwet(data[i], k))
                    else: # gas_unit == UNIT_PPMVWET
                        if k == 'q': qdry = data[i]
                        profs[p][k].append(ppmvdry2ppmvwet(data[i], k, qdry))
...
def make_div83(gui, gas_unit=UNIT_PPMVWET):
    profs = read_div83('ECMWF_83P_91L.dat', gas_unit=gas_unit)
