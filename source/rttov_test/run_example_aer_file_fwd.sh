...
AER_COEF_FILENAME="scaercoef_msg_3_seviri.dat"  # Location of this file is set below in $SCCOEF_DIR
...
rm -f $AER_COEF_FILENAME
if [ -s $SCCOEF_DIR/$AER_COEF_FILENAME ]; then
  ln -s $SCCOEF_DIR/$AER_COEF_FILENAME
else
  echo "Coef file $SCCOEF_DIR/$AER_COEF_FILENAME not found, aborting..."
  exit 1
fi
...
rm -f $COEF_FILENAME $AER_COEF_FILENAME
