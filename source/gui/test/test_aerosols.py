...
class Test(rttovgui_unittest_class.RttovGuiUnitTest):
    ...
    def test_aerosols(self):
        ...
        profile_with = self.p.config.ENV[
            "RTTOV_GUI_PROFILE_DIR"] + "/aer101lev_allgas.H5"
        ...
        for coefFile in list_aer:
            ...
            if os.path.exists(std):
                ...
                self.assertEqual(err, 0)
                self.check_option(self.p)
                self.assertTrue(self.p.myOption.statusOption["ADDAEROSL"])
                self.p.myOption["ADDAEROSL"] = True
                err = self.p.runDirect()
                self.assertEqual(err, 0)
                err = self.p.runK()
                self.assertEqual(err, 0)
                ...
                self.assertFalse(self.p.myOption.statusOption["ADDAEROSL"])
