...
# 2015-09-10 : pseudo run : minis and versus
#
"""
...
# modfif ordre 2015-09-03 : BT par defaut
        self.pltlist1 = ['BT','BT_CLEAR','TOTAL','CLEAR','REFL','REFL_CLEAR']
...
# 2 x onglets : Runs et Channels
        self.nbk = wx.Notebook(self, -1,wx.DefaultPosition,wx.DefaultSize,wx.NB_TOP)
...
# simulation lecture suivantes
    ...
    def initPlot111(self):
        pnl = wx.Panel(self.nbk, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.TAB_TRAVERSAL)
...
# choix : plots
        self.chxplt111 = wx.ComboBox(self.spb111, choices=self.pltlist, style=wx.CB_READONLY)
        self.chxplt111.SetSelection(0)
        self.chxplt111.Bind(wx.EVT_COMBOBOX, lambda event: self.pltChoix111(event,'type'))
        self.bxsh111.Add(self.chxplt111)

# choix : runlist init dans initPlotRun et maj dans addRun
        self.chxrun111 = wx.ComboBox(self.spb111, choices=self.runlist, style=wx.CB_READONLY)
        self.chxrun111.SetSelection(1)
        self.chxrun111.Bind(wx.EVT_COMBOBOX,lambda event: self.pltChoix111(event,'run'))
        self.bxsh111.Add(self.chxrun111)

# choix : run de reference
        self.chxrefrun111 = wx.ComboBox(self.spb111, choices=self.refrunlist, style=wx.CB_READONLY)
        self.chxrefrun111.SetSelection(0)
        self.chxrefrun111.Bind(wx.EVT_COMBOBOX,lambda event: self.refrunChoix(event,'111'))
        self.bxsh111.Add(self.chxrefrun111)


# choix : prunlist maj dans addRun
        self.chxprun111 = wx.ComboBox(self.spb111, choices=self.prunlist, style=wx.CB_READONLY)
        self.chxprun111.SetSelection(0)
        self.chxprun111.Bind(wx.EVT_COMBOBOX,lambda event: self.pltChoix111(event,'prun'))
...
# choix : chnlist init dans initPlotChn
        if self.nbChn < 50:
            self.chxchn111 = wx.ComboBox(self.spb111, choices=self.chnlist, style=wx.CB_READONLY)
            self.chxchn111.SetSelection(0)
            self.chxchn111.Bind(wx.EVT_COMBOBOX,lambda event: self.pltChoix111(event,'chn'))
            self.bxsh111.Add(self.chxchn111)

# choix : pchnlist maj dans addPChn
        if self.nbChn < 50:
            self.chxpchn111 = wx.ComboBox(self.spb111, choices=self.pchnlist, style=wx.CB_READONLY)
            self.chxpchn111.SetSelection(0)
            self.chxpchn111.Bind(wx.EVT_COMBOBOX,lambda event: self.pltChoix111(event,'pchn'))
...
# init taby : diff ref-i
                self.taby['TOTAL_( %s )' % nprun] = self.ttotal[i-1][:] - self.ttotal[iref-1][:]
                self.taby['CLEAR_( %s )' % nprun] = self.tclear[i-1][:] - self.tclear[iref-1][:]
                self.taby['BT_( %s )' % nprun] = self.tbt[i-1][:] - self.tbt[iref-1][:]
                self.taby['BT_CLEAR_( %s )' % nprun] = self.tbtclear[i-1][:] - self.tbtclear[iref-1][:]
                self.taby['REFL_( %s )' % nprun] = self.trefl[i-1][:] - self.trefl[iref-1][:]
                self.taby['REFL_CLEAR_( %s )' % nprun] = self.treflclear[i-1][:] - self.treflclear[iref-1][:]
...
# init taby : i
                ...
                self.taby['REFL_CLEAR_( %s )' % nprun] = self.treflclear[i-1][:]
...
# pas de plot si tous points NaN
        if nbnan<len(tx1):
            nplt.plot(tx1, self.taby[label], self.cols_d[typPlot], markersize=self.mz, label='')
        else:
            txt = "%s : Nan" % label
...
# axe X inferieur en WN 
        ...
        if typAxe=='prun':
            ...
            nplt.plot(tx1, self.taby[ref], self.cols_d[typPlot], markersize=self.mz)
            ...
            nplt.plot(tx1, self.taby[label111], self.cols_d[typPlot], markersize=self.mz)
            nplt.set_xlim(numpy.nanmin(tx1), numpy.nanmax(tx1))
        ...
        if oklabel:
            ...
            nplt.text(0.015, 0.99, legend1, color=self.txtcol[typPlot], transform=nplt.transAxes, fontsize=14, verticalalignment='bottom', bbox=txtbox)
            nplt.text(0.80, 0.98, legend2, color='red', transform=nplt.transAxes, fontsize=12, verticalalignment='bottom', bbox=txtbox)
    ...
    def redraw111(self,i_plt,i_run,i_prun,i_chn,i_pchn):
        ...
        #print "taxes : ",taxes
        if i_run >= 0 :
            taxes[0] = self.majSubplot111(taxes[0], taxes[1], 'run', self.pltlist[i_plt] ,i_run+1, self.nbChn+1)
        elif i_prun >= 0 :
            taxes[0] = self.majSubplot111(taxes[0], taxes[1], 'prun', self.pltlist[i_plt] ,i_prun+1, self.nbChn+1)
        elif i_chn >= 0 :
            taxes[0] = self.majSubplot111(taxes[0], taxes[1], 'chn', self.pltlist[i_plt] ,i_chn+1, self.nbRun+1)
        elif i_pchn >= 0 :
            taxes[0] = self.majSubplot111(taxes[0], taxes[1], 'pchn', self.pltlist[i_plt] ,i_pchn+1, self.nbRun+1)
...
# init taby
        ...
        self.chxrun = wx.ComboBox(self.spbr, choices=self.runlist, style=wx.CB_READONLY)
        self.chxrun.SetSelection(1)
        self.chxrun.Bind(wx.EVT_COMBOBOX,self.runChoix)
        self.bxshr.Add(self.chxrun)
# choix run de reference
        self.chxrefrun = wx.ComboBox(self.spbr, choices=self.refrunlist, style=wx.CB_READONLY)
        self.chxrefrun.SetSelection(0)
        self.chxrefrun.Bind(wx.EVT_COMBOBOX,lambda event: self.refrunChoix(event,'run'))
        self.bxshr.Add(self.chxrefrun)

# 1 x bouton par Rnu(i) - Run(i-1)
        self.chxprun = wx.ComboBox(self.spbr, choices=self.prunlist, style=wx.CB_READONLY)
        self.chxprun.SetSelection(0)
...
# maj pruns
    ...
    def redrawRun(self,i_run):
        ...
        for iplt in range(len(self.pltlist)):
            taxes[iplt] = self.majSubplot(taxes[iplt], 'run', self.pltlist[iplt], i_run, self.nbChn+1)
    ...
    def redrawPRun(self, i_prun):
        ...
        taxes = self.rfig.get_axes()
        for iplt in range(len(self.pltlist)):
            taxes[iplt] = self.majSubplot(taxes[iplt], 'prun',  self.pltlist[iplt], i_prun, self.nbChn+1)
...
# 1 x bouton par chn
        ...
        self.chxchn = wx.ComboBox(self.spbc, size=(125,26), choices=self.chnlist, style=wx.CB_READONLY)
...
# 1 x bouton par pseudochn
        ...
        self.chxpchn = wx.ComboBox(self.spbc, size=(165,26), choices=self.pchnlist, style=wx.CB_READONLY)
...
# saisie formule
        ...
        self.txtF = wx.TextCtrl(self.spbc, id=-1, value=formule, size=(250,30))
        self.txtF.SetFont(wx.Font(12, wx.SWISS, wx.NORMAL, wx.BOLD, False, "Arial"))
...
# validation minimale !
    ...
    def redrawChn(self,i_chn):
        ...
        for iplt in range(len(self.pltlist)):
            taxes[iplt] = self.majSubplot(taxes[iplt], 'chn', self.pltlist[iplt], i_chn, self.nbRun+1)
...
# recherche de la valeur reel la plus proche
    ...
    def redrawPChn(self,i_pchn):
        ...
        taxes = self.cfig.get_axes()
        for iplt in range(len(self.pltlist)):
            taxes[iplt] = self.majSubplot(taxes[iplt], 'pchn', self.pltlist[iplt], i_pchn, self.nbRun+1)
        ...
        if event.inaxes:
            ...
            try:
                ...
                if re.search('formula',ax):
                    ...
                    y = self.taby[bx][x-1]
                    txt = 'run #%02g Y=%g %s' % (x, y, self.units_d[ax.split('_')[0]])
                elif re.search('chn',ax):
                    c = ax[-2:]
                    y = self.taby[ax][x-1]
                    if self.lstWv[int(c)-1] >= 400.0:
                        txt0 = '%s=%.3f %sm]' % (u'\u03BB', (10000/self.lstWv[int(c)-1]), u'\u03BC')
                    else:
                        txt0 = '%s=%.4f GHz]' % (u'\u03BD', (spl*self.lstWv[int(c)-1]/1000000000))

                    txt = 'run #%02g Y=%g %s [wn=%.3f cm-1 / ' % (x, y, self.units_d[ax.split('_')[0]], self.lstWv[int(c)-1])+txt0
# cas run et prun
...
# cas run
                    if ax.count('run')==1:
                        ...
                        if self.nbChn < 50:
                            ...
                            txt = 'chn #%02g Y=%g %s [wn=%.3f cm-1 / ' % (nx, y, self.units_d[ax.split('_')[0]], self.lstWv[nx-1])
                            ...
                            y = self.taby[ax][nx]
                            #print "mouse : ", x, nx 
                            txt = '(chn #%02g) X=%.3f cm-1 Y=%g %s [' % (nx+1,vx, y, self.units_d[ax.split('_')[0]])
                            #print "texte statusbar :",txt
 
                        if self.lstWv[nx] >= 400.0:
                            txt0 = '%s=%.3f %sm]' % (u'\u03BB', (10000/self.lstWv[nx]), u'\u03BC')
                        else:
                            txt0 = '%s=%.4f GHz]' % (u'\u03BD', (spl*self.lstWv[nx]/1000000000))
...
# cas prun
                        ...
                        if re.search('minus',ax):
                            txt = '(chn #%02g) X=%.3f cm-1 Y_%s=%g Y_minus=%g ' % (nx+1,vx,'%s' % ref[-2:],yf,yr-yf) 
                        else:
                            txt = '(chn #%02g) X=%.3f cm-1 Y_%s=%g Y_%s=%g ' % (nx+1,vx,'%s' % ref[-2:],yf,'%s' % run[-2:],yr) 
...
#
        ...
        if len(self.lstChn) == 0 or self.nbRun == 0:
            self.sat = rsat
            self.ins = rins
            self.txtsb = '%s / %s' % (self.sat.replace(' ', '').upper(), self.ins.upper())
            ...
            if rsat != self.sat or rins != self.ins:
                txt = 'Attn : %s/%s --> %s/%s' % (self.sat, self.ins, rsat, rins)
                self.writeSB(txt, 'RED', 10, 1)
                self.nbRun -= 1
                return
...
#
            ...
            if self.pccomp[self.nbRun]:
                self.ttotal = numpy.concatenate((self.ttotal, [rad['TOTAL_PCCOMP']]))
                self.tbt = numpy.concatenate((self.tbt, [rad['BT_PCCOMP']]))
                self.tclear = numpy.concatenate((self.tclear, numpy.zeros((1,self.nbChn))))
                self.tbtclear = numpy.concatenate((self.tbtclear, numpy.zeros((1,self.nbChn))))
                self.trefl = numpy.concatenate((self.trefl, numpy.zeros((1,self.nbChn))))
                self.treflclear = numpy.concatenate((self.treflclear, numpy.zeros((1,self.nbChn))))
            else:
#
# cas monocanal
#
                if self.nbChn < 2:
                        self.ttotal = numpy.concatenate((self.ttotal, [[rad['TOTAL']]]))
                        self.tclear = numpy.concatenate((self.tclear, [[rad['CLEAR']]]))
                        self.tbt = numpy.concatenate((self.tbt, [[rad['BT']]]))
                        self.tbtclear = numpy.concatenate((self.tbtclear, [[rad['BT_CLEAR']]]))
                        self.trefl = numpy.concatenate((self.trefl, [[rad['REFL']]]))
                        self.treflclear = numpy.concatenate((self.treflclear, [[rad['REFL_CLEAR']]]))
                else:
                	self.ttotal = numpy.concatenate((self.ttotal, [rad['TOTAL']]))
                	self.tclear = numpy.concatenate((self.tclear, [rad['CLEAR']]))
                	self.tbt = numpy.concatenate((self.tbt, [rad['BT']]))
...
# fin init
            ...
            self.addRuns()
            self.chxprun111.SetSelection(0)
            self.redraw111(self.chxplt111.GetSelection(),self.nbRun-1,-1,-1,-1)
...
# maj taby canaux
        for ichn in range(1, self.nbChn+1):
            self.taby['TOTAL_chn_%02d' % ichn] = numpy.transpose(self.ttotal)[ichn-1][:]
            self.taby['CLEAR_chn_%02d' % ichn] = numpy.transpose(self.tclear)[ichn-1][:]
            self.taby['BT_chn_%02d' % ichn] = numpy.transpose(self.tbt)[ichn-1][:]
            self.taby['BT_CLEAR_chn_%02d' % ichn] = numpy.transpose(self.tbtclear)[ichn-1][:]
            self.taby['REFL_chn_%02d' % ichn] = numpy.transpose(self.trefl)[ichn-1][:]
            self.taby['REFL_CLEAR_chn_%02d' % ichn] = numpy.transpose(self.treflclear)[ichn-1][:]
            ...
            for irun in range(1,self.nbRun+1):
                ...
                for ichn in range(1,self.nbChn+1):
                    ...
                    if re.search(nchn,ttotaleval):
                        ...
                        itbtclear = 'self.tbtclear[%d][%d]' % (irun-1, ichn-1)
                        ...
                        itreflclear = 'self.treflclear[%d][%d]' % (irun-1, ichn-1)
                        treflcleareval = treflcleareval.replace(nchn,itreflclear)

                self.taby['TOTAL_( formula_%02d )' % ipchn].append(eval(ttotaleval))
                self.taby['CLEAR_( formula_%02d )' % ipchn].append(eval(tcleareval))

                self.taby['BT_( formula_%02d )' % ipchn].append(eval(tbteval))
                self.taby['BT_CLEAR_( formula_%02d )' % ipchn].append(eval(tbtcleareval))

                self.taby['REFL_( formula_%02d )' % ipchn].append(eval(trefleval))
                self.taby['REFL_CLEAR_( formula_%02d )' % ipchn].append(eval(treflcleareval))
...
if __name__ == "__main__":
    ...
    app = wx.App(0)
    frame = RadianceFrame(None,"Radiance Viewer",fh5[1],0)
    frame.Show()
