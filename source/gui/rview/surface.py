...
class SurfaceView(util.GenericView):
    ...
    def __init__(self, parent, project):
        ...
        self.SetSizer(sizer)
        self.CreateMenuBar()
        self.SetSize((1020, 700))
        self.SetMinSize((1020, 700))
    ...
    def _MakeBinding(self):
        ...
        self.applyBtn.Bind(wx.EVT_BUTTON, self.OnApplyChange)
    ...
    def ShowSurfaceParameters(self):
        ...
        for label in self.myProfile.profileParamList[3:]:
            ...
            if (label == 'IDG' or label == 'ISH'):
                self.ct[label] = wx.ComboBox(
                    self.panel1,
                    choices=self.myProfile.typeListe[label].values())
            else:
        ...
        for label in self.myProfile.sskin_list[:-1]:
            ...
            if (label == 'SURFTYPE' or label == 'WATERTYPE'):
                self.ct['SKIN_' + label] = wx.ComboBox(
                    self.panel1, choices=self.myProfile.typeListe[
                                                            label].values())
            else:
        ...
        self.gbs.Add(wx.StaticText(self.panel1, -1, 'Fastem parameters'),
                     ...
                     span=(1, 2),
                     flag=wx.ALIGN_CENTER | wx.ALIGN_BOTTOM, border=8)
        self.cb = wx.ComboBox(self.panel1, choices=self.valFastem.keys())
        line += 1
    ...
    def GetProfileValues(self, aProfile=None):
        ...
        try:
            ...
            for param in self.myProfile.profileParamList[3:]:
                if (param == 'IDG' or param == 'ISH'):
                    val = self.ct[param].GetValue()
                    self.myProfile[param] = [
                        k for (k, v) in self.myProfile.typeListe[
            ...
            if (param == 'SURFTYPE' or param == 'WATERTYPE'):
                ...
                self.ct[
                    'SKIN_' + param].SetValue(float(
                                        self.myProfile['SKIN'][param]))
        for key in self.valFastem.keys():
            value = 'Default'
...
if __name__ == "__main__":
    ...
    p.openProfile(prefix + "/rttov_tests/AA_preproCf_44L.H5", 1)
    p.saveProfile(prefix + "/rttov_tests/my_profile.h5")
    print "load coefficient"
    coefFile = prefix + "/rttov_tests/rtcoef_metop_2_avhrr.dat"
    p.myCoeffs.fileName["standard"] = coefFile
    # p.loadCoefficients()
    print p.myCoeffs.nchannels
    p.myCoeffs.nchannels = 22
    print p.myCoeffs.nchannels

    p.openSurface(prefix + "/rttov_tests/surface_22channels.H5")
    ex = wx.App()
