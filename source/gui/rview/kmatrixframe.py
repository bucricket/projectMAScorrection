...
class MyEmptyCustomToolbar(ToolBar):
    toolitems = [t for t in ToolBar.toolitems if
                 t[0] in ()]
...
#
        ...
        self.tlb.AddSimpleTool(self.SCALE10K_ID, _load_bitmap(
            os.environ["RTTOV_GUI_PREFIX"] + '/icons/k10.png'),
            'scale Kx10%', '')
        ...
        self.tlb.AddSimpleTool(self.RESETK_ID, _load_bitmap(
            os.environ["RTTOV_GUI_PREFIX"] + '/icons/reset.png'),
            'reset K', '')
...
#
        ...
        self.tlb.AddSimpleTool(self.SELCHAN_ID, _load_bitmap(
            os.environ["RTTOV_GUI_PREFIX"] + '/icons/kP.png'),
            'select channnel', '')
...
#
        ...
        self.draw_plot('T', 'T', self.kmat.kmatrix[
        ...
        self.draw_plot('Q', 'Q', self.kmat.kmatrix[
...
#
        if self.gasplot:
            ...
            self.draw_plot('G', gas, self.kmat.kmatrix[
...
#
    ...
    def onselchan(self, evt):
        ...
        if (chnbox.ShowModal() == wx.ID_OK):
            ...
            pchan = self.kmat.getchanprof(c - 1)
            self.mykProfileView = rview.kprofileframe.KProfileView(
                self, pchan, channel=c,
                baseProfile=self.baseProfile, runNumber=self.runNumber)
        ...
        for ax in range(0, len(taxes)):
            if taxes[ax].get_title():
                ...
                for yl in range(0, len(taxes[ax].get_yticks())):
                    ylabs[yl] = (int(round(taxes[ax].get_yticks()[yl] *
                                 self.lev/levx)))
...
if __name__ == "__main__":
    ...
    app = wx.App(0)
    frame = KMatrixFrame(None, "K Matrix Viewer", fh5[1])
    frame.Show()
