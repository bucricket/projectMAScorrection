...
Profile::Profile(int nlevels):gas_units(unknown),verbose(true),angles{},datetimes{},s2m{},
        skin{},surfgeom{},surftype{},simplecloud{},zeeman{},icecloud{},
        P{},T{},items{} {
            // constructor
        ...
        ///@param gasUnits unknown=-2,ppmv_dry=-1,compatibility_mode=0,kg_per_kg=1,ppmv_wet=2
        ...
        ///@param ish, idg
        void Profile::setIceCloud(const int ish, const int idg) {

            if ( !icecloud.empty() ) icecloud.erase(icecloud.begin(),icecloud.end());
            icecloud.push_back(ish);
        ...
        /// if simplecoud, icecloud or zeeman have not been set initialise them with default values
        bool Profile::check() {
            bool ok=true;
            if ( this->P.size() > 0 ) {
